...
class SoftPlusWithGradientFloorFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_output: torch.Tensor):
        ...
        if ctx.needs_input_grad[0]:
            grad_x = grad_output * (
                grad_floor + (1.0 - grad_floor) / (1.0 + torch.exp(-x))
            )
...
class PolicyHead(torch.nn.Module):
    def __init__(self, c_in, c_p1, c_g1, config, activation):
        ...
        if config["version"] <= 14:
            ...
            self.linear_pass2 = torch.nn.Linear(
                c_p1, self.num_policy_outputs, bias=False
            )
...
class MetadataEncoder(torch.nn.Module):
    def __init__(self, config: modelconfigs.ModelConfig):
        ...
        self.c_input = modelconfigs.get_num_meta_encoder_input_features(
            self.meta_encoder_version
        )
        ...
        self.linear_output_to_trunk = torch.nn.Linear(
            self.c_internal, self.c_trunk, bias=False
        )
    ...
    def initialize(self):
        ...
        with torch.no_grad():
            ...
            init_weights(
                ...
                self.activation,
                scale=bias_scale,
                fan_tensor=self.linear1.weight,
            )
            ...
            init_weights(
                self.linear_output_to_trunk.weight, self.activation, scale=weight_scale
            )
    ...
    def postprocess_single_heads_output(self, outputs):
        ...
        if self.config["version"] < 14:
            pred_shortterm_value_error = (
                SoftPlusWithGradientFloorFunction.apply(
                    out_moremiscvalue[:, 0], 0.05, False
                )
                * self.shortterm_value_error_multiplier
            )
            pred_shortterm_score_error = (
                SoftPlusWithGradientFloorFunction.apply(
                    out_moremiscvalue[:, 1], 0.05, False
                )
            ...
            pred_shortterm_value_error = (
                SoftPlusWithGradientFloorFunction.apply(
                    out_moremiscvalue[:, 0], 0.05, True
                )
                * self.shortterm_value_error_multiplier
            )
            pred_shortterm_score_error = (
                SoftPlusWithGradientFloorFunction.apply(
                    out_moremiscvalue[:, 1], 0.05, True
                )
                * self.shortterm_score_error_multiplier
            )
